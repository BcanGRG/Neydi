...
**İlerleme:** 22 / 66 — *Faz 4 sürüyor. Sırada 4.6 (Fiş Kontrol ekranı) — ayrıştırıcı ilk kez gerçek fişte koşacak ve 4.4/4.5 işaretleri kapanabilir.*
...
- [~] **0.2 — Fiş ayrıştırma ölçümü** *(sende bekliyor)*. **Sorusu değişti:** artık "hangi model daha iyi okuyor" değil, "**parser senin gerçek fişlerinde ne kadar tutuyor**". `ANTHROPIC_API_KEY` gerekmiyor. Akış: cihazda ML Kit'in ürettiği **ham metin satırlarını** bir kere dök, sonra parser'ı o dökümlere karşı `commonTest`'te geliştir. Ölçüt aritmetik kapısı: kaç fiş `DOGRULANDI`, kaç fiş `TUTARSIZ`. İki fiş fotoğrafı hâlâ `tools/receipt-eval/receipts/` bekliyor.
...
- [~] **4.5 — Aritmetik kapı.** Mantık hazır: `aritmetikTutuyorMu()` — Σ(ürün) − Σ(indirim) = TOPLAM, ±5 kuruş. **KDV eklenmiyor ve çıkarılmıyor** (0.3'te düzeltilen formül). Üç durumlu dönüyor: tuttu / tutmadı / **toplam okunamadı** — "doğrulanamadı" ile "tutmadı" ayrı şeyler, ikisini `false`'a katlamak fişi haksız yere şüpheli gösterirdi. **Yeşil/amber çip henüz yok** (4.6 ekranıyla gelecek), o yüzden `[~]`.
