...
Tek gerçek kaynak. 11 faz, **95 adım**. Her adım bir PR.
...
**İlerleme:** 43 / 95 — *43 kapandı · 4 kod tamam, cihaz doğrulaması bekliyor · 48 açık.* **Şema v3 yapıldı (F2.8)**, yani Faz 5/6 artık veri yazabilir. **F6.8 bitti**, yani "Her zamankiler" artık dolabiliyor ve Ekran 3 açılabilir hale geldi. **F6.3 bitti** — tek şerit iki modda: boşken öneri, yazarken tamamlama; isabet artık `fromSuggestion` ile ölçülüyor. **Sırada F6.4** (Eksik Olabilir / Ekran 3): motor ve `isStaple` hazır, ekranın önündeki engeller `onGoShopping`'in bağlanması ve F6.1'in gezi-ötesi verisinin bölümlere dökülmesi. Uzun fiş konforu **F4.13** olarak eklendi — kullanıcının ana marketi uzun fiş basıyor, bu ana akış.
...
- [ ] **4.13 — Uzun fiş: parça-parça akışı tek dokunuşa indir** *(kullanıcının ana marketi AKYURT uzun fiş basıyor — bu bir kenar durum değil, ana akış)*. **Fizik değişmedi:** ~60 kalemlik fiş tek karede satır başına **4,7 piksel** ve ML Kit 60 satırın 2'sini okudu; ham kamera çözünürlüğü bile ~7 px/satır verirdi. Çözüm yazılım değil, **çekim biçimi**: karede 10-15 satır.
  **Bugün çalışan:** aynı geziye birden fazla fiş bağlanabiliyor ve parçalar bugün de işliyor — her parçanın satırları fiyat hafızasına giriyor; **toplam yalnızca son parçada** olduğu için gezi toplamı da doğru çıkıyor (ara parçalar `null`, `SUM()` onları atlıyor). Okunamayan karede `UNREADABLE_MESSAGE` zaten *"parça parça çek"* diyor.
  **Eksik olan konfor + iki pürüz:** (1) mesajdan sonra **"Parçayı çek"** düğmesi yok — kullanıcı Liste'ye dönüp özet kartını yeniden bulmak zorunda; akış tek dokunuş olmalı. (2) Ara parçalar toplamsız olduğu için **MISMATCHED** görünüyor — dürüst ama yanlış yönlendirici; "bu bir parça" bilgisi taşınırsa amber çip yerine *"parça 1/3"* denebilir. (3) Tasarımın kamera overlay metni (*"Uzunsa 2 kare çek"*) sistem kamerasında gösterilemiyor; fiş çekme sheet'ine taşınabilir.
  **Kabul ölçütü:** AKYURT'un gerçek uzun fişi 3-4 parçada çekilir, her parça okunur, gezi toplamı fişin gerçek toplamına eşit çıkar ve hiçbir parça kullanıcıya hata gibi görünmez.
