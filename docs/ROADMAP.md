...
**İlerleme:** 37 / 93 — *37 kapandı · 4 kod tamam, cihaz doğrulaması bekliyor · 52 açık.* **Şema v3 yapıldı (F2.8)**, yani Faz 5/6 artık veri yazabilir. **F6.8 bitti**, yani "Her zamankiler" artık dolabiliyor ve Ekran 3 açılabilir hale geldi. **F6.1 bitti** — istatistik hem listeden hem fişten sayıyor. **Sırada F6.2** (skor formülü); `muAdjust` ve `takeOutcome` kolonları v3'te hazır, ama *"unuttum"* ile *"gerekmedi"* ayrımının UI'ı F4.12'de duruyor.
