 ...
 * parcasi degil.
 */
private val MADDE_ONEKI = Regex("""^\s*(?:[-*•·–—]+|\d+[.)])\s*""")

/** Emoji ve onay isaretleri: kopyalanan listelerde sik ve ada ait degil. */
private val ISARETLER = Regex("""[\u2705\u2714\u274C\u2611\u25A0\u25A1\u2610\uD83D\uDFE2\uD83D\uDD34]""")

fun panoSatirlari(metin: String): List<String> =
    metin.lineSequence()
        .map { it.replace(ISARETLER, "").replace(MADDE_ONEKI, "").trim() }
 ...
 */
fun panoListeMi(metin: String?): Boolean =
    metin != null && panoSatirlari(metin).size >= 3
